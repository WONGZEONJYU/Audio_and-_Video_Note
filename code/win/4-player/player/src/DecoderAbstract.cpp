...
DecoderAbstract::DecoderAbstract(DecoderAbstract::Cv_Any_Type &cv,
                                ...
                                m_cv{cv},m_pkt_queue{pkt_q},m_frame_queue{frame_q},m_avcodec_ctx{&av_codec_ctx} {
}
...
void DecoderAbstract::av_decoder_abort() noexcept(true) {
    frame_queue_signal(&m_frame_queue);
    packet_queue_abort(&m_pkt_queue);
    packet_queue_flush(&m_pkt_queue);
}
...
void DecoderAbstract::av_decoder_start(void *args) noexcept(true){
    packet_queue_start(&m_pkt_queue);
...
int DecoderAbstract::decode_frame(AVFrame *frame) {
    ...
    while (true){
        ...
        if (packet_queue_get(&m_pkt_queue,&m_pkt,1,&m_pkt_serial)< 0){ //阻塞读取
            return -1;
        }
