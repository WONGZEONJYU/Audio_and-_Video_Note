...
void VideoDecoder::av_decoder_thread(void *o) {
    ...
    try {
        ...
        const auto frame_rate (av_guess_frame_rate(obj->f_format_ctx(), obj->f_video_st(), nullptr));
        //frame_rate出来的结果与time_base刚好是相反的,例如:time_base = 1/24 ,那么frame_rate = 24/1
        //int64_t pos{};
        ...
        while (true){
            ...
            //此处可能容易理解错误
            const auto duration_{ (frame_rate.den && frame_rate.num) ? av_q2d({frame_rate.den,frame_rate.num}):0 };

            const auto pts_{static_cast<double >((*frame)->pts)};

            const auto pts{AV_NOPTS_VALUE == pts_ ? NAN : pts_ * av_q2d(tb)};

            ret = queue_picture(*frame,pts,duration_,{},Pkt_Serial());
            if (ret < 0){
                throw std::runtime_error(std::string(__FUNCTION__ ) + " " + std::to_string(__LINE__) + " " + AVHelper::av_get_err(ret) + "\n");
            }
        }
