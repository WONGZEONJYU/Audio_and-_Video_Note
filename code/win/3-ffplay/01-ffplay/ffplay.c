    ...
    do {
        ...
        if (got_frame) { //解码成功
                ...
                if (reconfigure) { //预设的滤镜参数和实际解码出来的帧的参数不一样
                    ...
                    is->audio_filter_src.fmt            = frame->format; //更新滤镜的采样格式
...
static int stream_component_open(VideoState *is, int stream_index)
{
    ...
    const AVCodec *codec = avcodec_find_decoder(avctx->codec_id); //根本流提供的信息寻找匹配的解码器
...
{
    VideoState *is = av_mallocz(sizeof(VideoState)); /* 分配VideoState并初始化 */
