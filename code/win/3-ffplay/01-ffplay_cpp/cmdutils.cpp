...
void show_help_children(const AVClass *class_, int flags)
{
    ...
    while (child = av_opt_child_class_iterate(class_, &iter))
                        ...
                        const char *arg, const OptionDef *defs)
{
    ...
    if (po->type == OPT_TYPE_STRING) {
        ...
        *(int64_t *)dst = num;
        ...
        ret = av_parse_time(static_cast<int64_t*>(dst), arg, 1);
        if (ret < 0) {
        ...
        *(double *)dst = num;
        ...
        if (ret < 0) {
            ...
            av_log(nullptr, AV_LOG_ERROR,
                   ...
                   arg, opt, av_make_error_string(errbuf,AV_ERROR_MAX_STRING_SIZE,ret));
            goto finish;
        }
    }
...
static void dump_argument(FILE *report_file, const char *a_)
{
    const unsigned char *p,*a {reinterpret_cast<decltype(a)>(a_)};
                              ...
                              const OptionGroupDef *groups, int nb_groups)
{
    ...
    octx->groups    = static_cast<decltype(octx->groups)>(av_calloc(nb_groups, sizeof(*octx->groups))) ;
    if (!octx->groups)
    ...
    if (is_path) {
        ...
        if (datadir)
        {
            ...
            if (ls = strrchr(datadir, '/'))
            {
                ...
                char *new_datadir = static_cast<decltype(new_datadir)>(av_realloc_array(
                                ...
                                AVDictionary ***dst)
{
    ...
    *dst = NULL;
    ...
    opts = static_cast<decltype(opts)>(av_calloc(s->nb_streams, sizeof(*opts)));
    if (!opts)
        return AVERROR(ENOMEM);

    for (int i = 0; i < s->nb_streams; i++) {
        ret = filter_codec_opts(codec_opts, s->streams[i]->codecpar->codec_id,
                                s, s->streams[i], nullptr, &opts[i]);
        if (ret < 0)
            goto fail;
    }
    *dst = opts;
...
int grow_array(void **array, int elem_size, int *size, int new_size)
{
    ...
    if (*size < new_size) {
        uint8_t *tmp = static_cast<decltype(tmp)>(av_realloc_array(*array, new_size, elem_size));
        if (!tmp)
            return AVERROR(ENOMEM);
