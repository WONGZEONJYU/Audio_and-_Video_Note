...
{
    const AVCodecDescriptor * const *da = static_cast<decltype(da)>(a);
    const AVCodecDescriptor * const *db = static_cast<decltype(db)>(b);
...
static int get_codecs_sorted(const AVCodecDescriptor ***rcodecs)
{
    ...
    if (!(codecs = static_cast<decltype(codecs)>(av_calloc(nb_codecs, sizeof(*codecs))) )){
        return AVERROR(ENOMEM);
    }
...
int show_filters(void *optctx, const char *opt, const char *arg)
{
    ...
    while ((filter = av_filter_iterate(&opaque))) {
        ...
        for (int i = 0; i < 2; i++) {
            ...
            unsigned nb_pads = avfilter_filter_pad_count(filter, i);
            int j;
            for (j = 0; j < nb_pads; j++) {
...
int show_layouts(void *optctx, const char *opt, const char *arg)
{
    ...
    printf("Individual channels:\n"
           "NAME           DESCRIPTION\n");
    for (i = 0; i < 63; i++) {
        av_channel_name(buf, sizeof(buf), static_cast<AVChannel>(i));
        if (strstr(buf, "USR")){
            continue;
        }

        av_channel_description(buf2, sizeof(buf2), static_cast<AVChannel>(i));
        printf("%-14s %s\n", buf, buf2);
    }
    ...
    while (ch_layout = av_channel_layout_standard(&iter)) {
            ...
            printf("%-14s ", buf);
            for (i = 0; i < 63; i++) {
                int idx = av_channel_layout_index_from_channel(ch_layout, static_cast<AVChannel>(i));
                if (idx >= 0) {
                    av_channel_name(buf2, sizeof(buf2), static_cast<AVChannel>(i));
                    printf("%s%s", idx ? "+" : "", buf2);
                }
            }
            printf("\n");
...
int init_report(const char *env, FILE **file)
{
    ...
    expand_filename_template(&filename,
                             static_cast<const char*>(av_x_if_null(filename_template, "%p-%t.log")), tm);
    av_free(filename_template);
