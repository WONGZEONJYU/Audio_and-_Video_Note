...
void XVideoWidget::initializeGL() {
    ...
    TRY_CATCH(CHECK_EXC(m_shader.reset(new QOpenGLShaderProgram(this))),
        PRINT_ERR_TIPS(GET_STR(new QOpenGLShaderProgram(this) error!));return);
    ...
    TRY_CATCH(
        CHECK_EXC(m_VAO.reset(new QOpenGLVertexArrayObject(this))),
        PRINT_ERR_TIPS(GET_STR(new QOpenGLVertexArrayObject(this) error!));return);
...
//    m_VAO->bind();
    ...
    TRY_CATCH(CHECK_EXC(m_VBO.reset(new QOpenGLBuffer(QOpenGLBuffer::VertexBuffer))),
        PRINT_ERR_TIPS(GET_STR(new QOpenGLBuffer(QOpenGLBuffer::VertexBuffer) error!)); return );
    m_VBO->create();
    qDebug() << "m_VBO->bind() = " << m_VBO->bind();
    m_VBO->allocate(ver_tex_coordinate, sizeof(ver_tex_coordinate));
    m_VBO->setUsagePattern(QOpenGLBuffer::StaticDraw);

    TRY_CATCH(CHECK_EXC(m_EBO.reset(new QOpenGLBuffer(QOpenGLBuffer::IndexBuffer))),
        PRINT_ERR_TIPS(GET_STR(new QOpenGLBuffer(QOpenGLBuffer::IndexBuffer) error!)); return );
    m_EBO->create();
...
void XVideoWidget::paintGL() {
   ...
   // qDebug() << "begin " << __FUNCTION__ ;
    ...
    IS_FALSE_(!m_yuv_datum.isEmpty(),return);

    for (const auto &item : m_yuv_datum){
    ...
    for(uint32_t i{};const auto &item:m_textureYUV) {
        //Y数据的长度m_w * m_h
        //U数据的长度 m_w * m_h / 4
        //V数据的长度 m_w * m_h / 4
        item->bind(i);
...
#endif
    for (const auto &item:m_textureYUV) {
        item->release();
    }
...
bool XVideoWidget::Init(const int &w,const int&h) noexcept(true) {
    ...
    IS_FALSE_(m_shader && m_VBO && m_EBO && m_VAO,
    ...
    for(int i {};auto &item:m_textureYUV) {
        ...
        TRY_CATCH(CHECK_EXC(item.reset(new QOpenGLTexture(QOpenGLTexture::Target2D))),cleanup();return {});
...
void XVideoWidget::Repaint(const XAVFrame &frame) {
...

    {
        ...
        IS_FALSE_(m_shader && m_VAO && m_VBO && m_EBO && !m_yuv_datum.isEmpty() && !m_textureYUV.isEmpty()
            ,PRINT_ERR_TIPS(GET_STR(Please call the Init() function first ));
            return);

        IS_FALSE_(frame.width == m_w && frame.height == m_h,
