...
XCodecParameters_sp new_XAVCodecParameters() noexcept(false) {
    XCodecParameters_sp obj;
    CHECK_EXC(obj = std::make_shared<XCodecParameters>());
    return obj;
}

XCodecParameters_sp new_XAVCodecParameters(const AVCodecContext *src,const AVRational &tb) noexcept(false) {
    XCodecParameters_sp obj;
    CHECK_EXC(obj = std::make_shared<XCodecParameters>(src,tb));
    return obj;
}

XCodecParameters_sp new_XAVCodecParameters(const AVCodecParameters *src,const AVRational &tb) noexcept(false){
    XCodecParameters_sp obj;
    CHECK_EXC(obj = std::make_shared<XCodecParameters>(src,tb));
    return obj;
}
