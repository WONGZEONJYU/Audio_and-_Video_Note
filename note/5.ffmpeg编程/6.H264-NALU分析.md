...
NALU结构单元的主体结构如下所示 ; ⼀个原始的H.264 NALU单元 **$\color{SkyBlue}{通常由}$**   **$\color{red}{\mathbf{[Start \\\ Code]\\\ [NALU \\\ Header]\\\ [NALU \\\ Payload]}}$** 三部分组成 , 其中 Start Code ⽤于标示这是⼀个NALU 单元的开始 ,必须是 "00 00 00 01" 或 "00 00 01" , 除此之外基本相当于⼀个NAL header + RBSP
