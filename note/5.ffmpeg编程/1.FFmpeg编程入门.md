...
| ------------------------------------------------------------ | ---- |
| int (*read_header)(struct AVFormatContext * );               | 方法 |
| int (*read_packet)(struct AVFormatContext *, AVPacket *pkt); | 方法 |
| int avformat_open_input(AVFormatContext ** ps, const char * filename,AVInputFormat * fmt, AVDictionary ** options); | 方法 |

## 11.2 AVCodecContext和AVCodec之间的关系

* `AVCodecContext` 和 `AVCodec` 之间的关系
...
| ------------------------------------------------------------ | ---- |
| int (*decode)(AVCodecContext * , void * outdata, int * outdata_size,AVPacket * avpkt); <br />int (*encode2)(AVCodecContext *avctx, AVPacket * avpkt, const AVFrame * frame, int * got_packet_ptr); | 方法 |
