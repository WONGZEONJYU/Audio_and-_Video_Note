...
8. 如果⼀个滤波实例的输入标识名或者输出标识名⼀直没有与其它滤波实例的输出标识名或者输⼊标识名相同 , 则表明这些为外部的输⼊输出 , 通常我们会为其接上buffersrc以及buffersink , 按照这种规则 , 上⾯的滤波过程可以被描绘成以下滤波图 : <img src="assets/image-20240507111614162.png" alt="image-20240507111614162" />  
...
上⾯主要讨论了如何创建滤波过程，不过要进⾏滤波还需要把帧传输进⼊该过程，并在滤波完成后从
该过程中提取出滤波完成的帧。
